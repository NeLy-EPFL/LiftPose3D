...
def read_data(path, session_id, cam_id):
    return scipy.io.loadmat(
        f"{path}/data_3d_e{str(session_id)}/{str(session_id)}_cam{str(cam_id+1)}_data.mat"
    )


def read_cam(path, session_id, cam_id):
    return scipy.io.loadmat(
        f"{path}/calibration_e{session_id}/hires_cam{cam_id+1}_params_rRDistort.mat"
    )
...
def load_data(par_train, return_frame_info=False):
    ...
    for session_id in session_id_list:
        mat = [
            read_data(par_train["data_dir"], session_id, cid)
            for (cid, _) in enumerate(cam_list)
        ]
