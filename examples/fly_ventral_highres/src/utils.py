...
def get_coords_in_dim(targets, dim):
    """
    Get the indices of a list of keypoints in spatial dimension dim
...
def remove_roots(data, vis, targets, n_dim ):
    ...
    if vis is not None:
        vis = np.array([item for item in list(vis) for i in range(n_dim)])
        vis = vis[dim_to_use]
