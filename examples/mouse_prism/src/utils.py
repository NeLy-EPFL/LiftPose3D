...
def normalization_stats(train_set):
  ...
  data_mean = np.nanmean(complete_data, axis=0)
  data_std  =  np.nanstd(complete_data, axis=0)
...
def get_coords_in_dim(targets, dim):
    ...
    if len(targets)>1:
      ...
      dim_to_use = np.sort( np.hstack(targets))
  
    dim_to_use = np.array(dim_to_use)
    if dim == 1:
      dim_to_use = np.sort( np.hstack(dim_to_use))
