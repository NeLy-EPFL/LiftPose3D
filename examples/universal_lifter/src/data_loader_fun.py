...
class data_loader(Dataset):
    def __init__(self, data_path, is_train=True, noise=None, eangle=False, predict=False):
        """
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            if self.eangle:
                eangle = self.train_stat_2d['eangle']
                ...
                outputs = transform.world_to_camera(outputs, rcams)
                inputs = transform.project_to_camera(outputs, cam_par=(f[0], f[1], s[0], s[1]))
                
                # anchor points to body-coxa (to predict legjoints wrt body-coxas)
                inputs, _ = utils.anchor_to_root( {'inputs': inputs}, self.train_stat_2d['roots'], self.train_stat_2d['target_sets'], 2)
                outputs, _ = utils.anchor_to_root( {'outputs': outputs}, self.train_stat_2d['roots'], self.train_stat_2d['target_sets'], 3)
                ...
                #remove roots
                inputs, _ = utils.remove_roots({'inputs': inputs}, self.train_stat_2d['target_sets'], 2)
                outputs, _ = utils.remove_roots({'outputs': outputs}, self.train_stat_2d['target_sets'], 3)
