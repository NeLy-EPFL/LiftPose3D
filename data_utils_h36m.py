...
def main():
    ...
    rcams = load_cameras(bpath=data_dir + '/cameras.h5')
...
def load_data( bpath, subjects, actions):
  """
  Loads 2d ground truth, and puts it in an easy-to-acess dictionary
...
def transform_world_to_camera(poses_set, cams, ncams=4 ):
    ...
    for t3dk in sorted( poses_set.keys() ):
      ...
      for c in range( ncams ):
        ...
        camera_coord = world_to_camera_frame( camera_coord, R, T) #transform
...
def project_to_cameras( poses_set, cams, ncams=4 ):
  ...
  for t3dk in sorted( poses_set.keys() ):
    ...
    for cam in range( ncams ):
      ...
      pts2d, _, = project_point_radial( np.reshape(t3d, [-1, 3]), R, T, f, c, k, p )
