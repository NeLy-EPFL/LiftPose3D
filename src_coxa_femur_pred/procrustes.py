...
def virtual_coxa_femur(data3d):
    d3d = np.zeros((data3d.shape[0], data3d.shape[1]+len(data_utils.OB_BODY_COXA), data3d.shape[2]))
    new_bodycoxa = [x+i for i, x in enumerate(data_utils.OB_BODY_COXA)]
    for new_bc, bc in zip(new_bodycoxa, data_utils.OB_BODY_COXA):
        d3d[:, new_bc] = data3d[:, bc]
        d3d[:, new_bc+1] = middle_point(data3d[:, bc], data3d[:, bc+1])
        d3d[:, new_bc+2:new_bc+5] = data3d[:,bc+1:bc+4]
    return d3d, new_bodycoxa

def remove_virtual_coxa_femur(data3d):
    for bc in data_utils.OB_BODY_COXA:
        data3d = np.delete(data3d, bc+1, 1)
    return data3d
...
def procrustes_separate(pts, template, reflection='best', verbose=False, scaling=True,
    ...
    return remove_virtual_coxa_femur(pts3d_proc)
