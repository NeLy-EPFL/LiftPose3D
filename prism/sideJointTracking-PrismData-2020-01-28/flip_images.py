...
if __name__ == '__main__':
    dirs = [d for d in listdir("labeled-data") if isdir("labeled-data/"+d) and not d.startswith('.')]
    dirs.sort()
    for d in dirs:
        d = "labeled-data/" + d + "/"
        print(d)
        f = pd.read_hdf(d+filename, 'df_with_missing')
        ...
        for idx, im_all_name in enumerate(f.index):
            ...
            if is_going_left(labels):
                ...
                cv2.imwrite(d + im_name, im_flipped)
        ...
        new_f.to_hdf(d + filename, 'df_with_missing', format='table', mode='w')
        print()
        print("[+] complete!")
        print()
